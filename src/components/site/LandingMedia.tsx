...
function SmartVideo({ src, title, hero = false }: { src: string; title: string; hero?: boolean }) {
  return (
    <video
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[.82fr_1.18fr]">
          <div className="relative z-10">
            ...
            <p className="mt-5 max-w-xl text-lg leading-8 text-muted-foreground">
              Kudi connects the counter, stockroom, customers and online store so every action has a useful next step.
            </p>
          ...
          <div className="relative min-h-[25rem] overflow-hidden rounded-[2.5rem] bg-foreground shadow-float sm:min-h-[34rem]">
            <SmartVideo src="https://pixabay.com/videos/download/x-23258_medium.mp4" title="Kudi retail workflow" hero />
            <div className="absolute inset-0 bg-gradient-to-r from-foreground/85 via-foreground/20 to-transparent" />
            <div className="absolute bottom-0 left-0 max-w-lg p-6 text-background sm:p-9">
              <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[.16em] text-accent"><Play className="size-3.5 fill-current" /> Kudi in motion</div>
              <p className="mt-3 font-display text-2xl font-bold sm:text-3xl">From busy counter to clear business picture.</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="grid items-center gap-12 lg:grid-cols-[1.05fr_.95fr]">
            <div className="order-2 lg:order-1">
              <Illustration src={ILLUSTRATIONS.shopping} alt="Illustration of online shopping and digital storefront activity" className="mx-auto max-w-[34rem]" />
            </div>
          ...
          <div className="mt-24 grid items-center gap-12 lg:grid-cols-[.95fr_1.05fr]">
            <div>
              ...
              <div className="mt-8 flex flex-wrap gap-4 text-sm font-semibold">
                <span className="inline-flex items-center gap-2"><ShoppingBag className="size-4 text-accent-ink" /> Orders</span>
                <span className="inline-flex items-center gap-2"><PackageCheck className="size-4 text-accent-ink" /> Stock</span>
                <span className="inline-flex items-center gap-2"><BarChart3 className="size-4 text-accent-ink" /> Reports</span>
              </div>
            </div>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="grid items-center gap-12 lg:grid-cols-[.8fr_1.2fr]">
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              {VIDEOS.map((video) => (
                <article key={video.src} className="group relative min-h-[17rem] overflow-hidden rounded-[2rem] border border-background/10 bg-background/5">
                  <SmartVideo src={video.src} title={video.title} />
      ...
      <section className="overflow-hidden px-4 py-20 sm:px-6 sm:py-28">
        <div className="mx-auto max-w-7xl">
          <div className="grid items-center gap-12 lg:grid-cols-[1.1fr_.9fr]">
            ...
            <div>
              <p className="text-label-caps text-accent-ink">One source of truth</p>
              <h2 className="text-display-md mt-4">Your shop should not live in five different places.</h2>
              <p className="mt-5 text-lg leading-8 text-muted-foreground">Kudi gives your products, sales, customers, expenses and online orders one connected home.</p>
              <div className="mt-8 inline-flex items-center gap-2 font-semibold text-accent-ink"><Sparkles className="size-4" /> Less duplication. Better decisions.</div>
            </div>
          </div>

          <div className="mt-24 grid items-center gap-12 lg:grid-cols-[.9fr_1.1fr]">
            <div>
              <p className="text-label-caps text-accent-ink">Business intelligence</p>
              <h2 className="text-display-md mt-4">Turn daily activity into decisions.</h2>
              <p className="mt-5 text-lg leading-8 text-muted-foreground">Reports should answer what happened, what needs attention and what you should do next.</p>
              <div className="mt-8 inline-flex items-center gap-2 font-semibold text-accent-ink"><ArrowRight className="size-4" /> From numbers to next action</div>
            </div>
