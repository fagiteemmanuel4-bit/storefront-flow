      ...
      <section className="border-y border-border bg-foreground px-4 py-20 text-background sm:px-6 sm:py-28">
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-12 lg:grid-cols-[1fr_.85fr] lg:items-center">
            <div><p className="text-label-caps text-accent">Hardware & checkout</p><h2 className="text-display-md mt-4">Designed to work with the way you already sell.</h2><p className="mt-5 max-w-2xl text-lg leading-8 text-background/65">Use the camera when you are on a phone. Use a compatible external barcode or QR scanner when you are at a desktop counter. Build a receipt design that fits your brand and prepare for compatible printer connections.</p><div className="mt-8 grid gap-3 sm:grid-cols-2">{[[ScanLine, "Camera scanning"], [Laptop, "USB / Bluetooth PC scanners"], [Printer, "Receipt printer workflows"], [Receipt, "Custom receipt designer"]].map(([Icon, label]) => <div key={label as string} className="flex items-center gap-3 rounded-xl border border-background/10 bg-background/[.05] px-4 py-3 text-sm font-medium"><Icon className="size-4 text-accent" />{label as string}</div>)}</div><Link to="/support" className="mt-8 inline-flex items-center gap-2 font-semibold text-accent hover:underline">Read hardware guides <ArrowRight className="size-4" /></Link></div>
            <div className="rounded-[2rem] border border-background/10 bg-background/[.05] p-6 sm:p-8"><div className="flex items-center justify-between"><div><p className="text-xs uppercase tracking-[.16em] text-background/45">Counter setup</p><h3 className="mt-2 font-display text-2xl font-bold">Ready for the rush</h3></div><span className="flex size-11 items-center justify-center rounded-xl bg-accent text-accent-foreground"><Zap className="size-5" /></span></div><div className="mt-7 space-y-3">{["Scan product", "Add to current sale", "Take payment", "Update stock", "Print or share receipt"].map((item, index) => <div key={item} className="flex items-center gap-3 rounded-xl border border-background/10 bg-background/[.04] p-3"><span className="flex size-7 items-center justify-center rounded-full bg-accent text-xs font-bold text-accent-foreground">{index + 1}</span><span className="text-sm text-background/75">{item}</span><Check className="ml-auto size-4 text-accent" /></div>)}</div></div>
          </div>
        </div>
      </section>
