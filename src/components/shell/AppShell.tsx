    ...
    <header className="sticky top-0 z-40 border-b border-border/70 bg-surface/80 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-3 px-4 py-3 sm:px-6 lg:px-10">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <nav className="hidden items-center gap-1 rounded-full border border-border bg-secondary/60 p-1 lg:flex" aria-label="Primary navigation">{NAV.map((item) => <Link key={item.to} to={item.to} data-tour={item.tour} className={cn("rounded-full px-4 py-2 text-sm font-semibold transition-all", pathname === item.to ? "bg-surface text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}>{item.label}</Link>)}</nav>
          {pathname === "/products" && canImport && <div className="flex items-center gap-1"><Link to="/product-import" aria-label="Import products" className="touch-target flex items-center gap-2 rounded-full border border-accent/30 bg-accent-soft px-3 py-2 text-sm font-semibold text-accent-ink transition hover:-translate-y-0.5"><FileSpreadsheet className="size-4" /><span className="hidden sm:inline">Import</span></Link><Link to="/stock-sense" aria-label="Open Kudi Sense" className="touch-target flex items-center gap-2 rounded-full border border-accent/30 bg-accent-soft px-3 py-2 text-sm font-semibold text-accent-ink transition hover:-translate-y-0.5"><SparklesFallback /><span className="hidden sm:inline">Kudi Sense</span></Link></div>}
          {pathname === "/pos" && <Link to="/unit-sale" className="touch-target hidden items-center gap-2 rounded-full border border-accent/30 bg-accent-soft px-3 py-2 text-sm font-semibold text-accent-ink transition hover:-translate-y-0.5 sm:flex"><Package className="size-4" /><span>Pack / piece</span></Link>}
...
function SparklesFallback() { return <span className="relative flex size-4 items-center justify-center" aria-hidden><span className="absolute size-2 rotate-45 rounded-[2px] bg-current" /><span className="absolute -right-0.5 -top-0.5 size-1 rounded-full bg-current" /></span>; }
