...
import { BarChart3, CloudUpload, FileBarChart, FileSpreadsheet, Globe2, Package, ScanLine, Settings2, Users, Wifi, WifiOff, LogOut } from "lucide-react";
...
const NAV = [
  ...
  { to: "/products", label: "Stock", icon: Package, area: "products" },
...
export function AppShell({ title, children }: { title: string; children: ReactNode }) {
  ...
  return (
    <div className="kudi-app-shell min-h-screen bg-background pb-24 lg:pb-0">
      ...
      <header className="sticky top-0 z-40 border-b border-border/70 bg-surface/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-3 px-4 py-3 sm:px-6 lg:px-10">
          <Link to="/pos" className="flex items-center gap-2" aria-label="Strap home"><span className="flex size-8 items-center justify-center rounded-full bg-accent"><span className="size-3 rotate-45 rounded-[3px] bg-foreground" /></span><span className="font-display text-lg font-bold tracking-tight">STRAP.</span></Link>
          <div className="ml-auto flex items-center gap-2">
            ...
            <div className={cn("hidden items-center gap-1 rounded-full border px-3 py-2 text-xs font-semibold sm:flex", online ? "border-border bg-secondary/60 text-muted-foreground" : "border-accent/30 bg-accent-soft text-accent-ink")} title={online ? "Strap is online" : "Strap is offline"}>{online ? <Wifi className="size-3.5" /> : <WifiOff className="size-3.5" />}{queued > 0 ? <><span>{queued} queued</span><CloudUpload className="size-3.5" /></> : <span>{online ? "Online" : "Offline"}</span>}</div>
            <button type="button" aria-label="Open menu" aria-expanded={menuOpen} onClick={() => setMenuOpen(true)} className="touch-target group flex items-center gap-2 rounded-full border border-border bg-secondary px-3 py-2 transition-all hover:border-accent/30 hover:bg-accent-soft active:scale-95"><Settings2 className="size-4" /><span className="hidden text-sm font-semibold sm:inline">Menu</span></button>
          </div>
        </div>
      </header>
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-10 lg:py-10">
        <div className="mb-6 flex flex-wrap items-end justify-between gap-3 lg:mb-8"><div><p className="text-label-caps flex items-center gap-1.5 text-muted-foreground"><Globe2 className="size-3.5" aria-hidden />{store?.name ?? "Loading shop"}{branch ? ` · ${branch.name}` : ""}{staffActive ? ` · ${staffSession!.name}` : ""}</p><h1 className="mt-1 text-title-lg lg:text-display-md">{title}</h1></div></div>
        {children}
      </div>
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-surface/95 backdrop-blur lg:hidden" aria-label="Primary"><div className="mx-auto flex max-w-md items-stretch justify-around px-2 pb-[env(safe-area-inset-bottom)]">{visibleNav.slice(0, 5).map((item) => { const Icon = item.icon; const active = pathname === item.to; return <Link key={item.to} to={item.to} className={cn("touch-target flex flex-1 flex-col items-center justify-center gap-1 py-2 text-[11px] font-semibold", active ? "text-accent-ink" : "text-muted-foreground")}><span className={cn("flex h-8 w-14 items-center justify-center rounded-full", active ? "bg-accent-soft" : "bg-transparent")}><Icon className="size-5" /></span>{item.label}</Link>; })}</div></nav>
      <UpdatesSheet />
      <AppMenuSheet open={menuOpen} onOpenChange={setMenuOpen} store={store} branchName={branch?.name ?? null} role={effectiveRole as any} onSignOut={() => { setMenuOpen(false); setLogoutOpen(true); }} />
      <SecurityModal
        open={logoutOpen}
        onOpenChange={setLogoutOpen}
        title="Log out of Strap?"
        description="You will need to sign in again to access this store. Any saved cloud data remains safely stored in Supabase."
        footer={
          <>
            <Button variant="outline" onClick={() => setLogoutOpen(false)}>Stay signed in</Button>
            <Button variant="destructive" onClick={() => { setLogoutOpen(false); void handleSignOut(); }}><LogOut className="mr-2 size-4" />Log out</Button>
          </>
        }
      >
        <div className="rounded-2xl border border-border bg-secondary/40 p-4 text-sm leading-6 text-muted-foreground">
          Strap will close your current session and return you to the sign-in screen. This is a local account action; it does not delete your store or business data.
        </div>
      </SecurityModal>
    </div>
  );
