...
import { BarChart3, CloudUpload, FileBarChart, FileSpreadsheet, Package, ScanLine, Store, Wifi, WifiOff } from "lucide-react";
...
const NAV = [
  ...
  { to: "/customers", label: "Customers", icon: Store, tour: "customers" },
...
export function AppShell({ title, children }: { title: string; children: ReactNode }) {
  ...
  const [menuOpen, setMenuOpen] = useState(false); const [online, setOnline] = useState(true); const [queued, setQueued] = useState(0);
  const pathname = useRouterState({ select: (s) => s.location.pathname }); const canImport = role === "owner" || role === "manager";
  ...
  return <div className="min-h-screen bg-background pb-24 lg:pb-0">
    <header className="sticky top-0 z-40 border-b border-border/70 bg-surface/80 backdrop-blur-xl"><div className="mx-auto flex max-w-7xl flex-wrap items-center gap-3 px-4 py-3 sm:px-6 lg:px-10">
      <Link to="/pos" className="flex items-center gap-2"><span className="flex size-8 items-center justify-center rounded-full bg-accent"><span className="size-3 rotate-45 rounded-[3px] bg-foreground" /></span><span className="font-display text-lg font-bold tracking-tight">KUDI.</span></Link>
      <div className="ml-auto flex items-center gap-2">
        ...
        <button type="button" aria-label="Open menu" aria-expanded={menuOpen} data-tour="menu" onClick={() => setMenuOpen(true)} className="touch-target group flex items-center gap-2 rounded-full border border-border bg-secondary px-3 py-2 transition-all hover:bg-accent-soft active:scale-95"><span className="flex flex-col items-center justify-center gap-[3px]"><span className="block h-[2px] w-4 rounded-full bg-foreground" /><span className="block h-[2px] w-4 rounded-full bg-foreground" /><span className="block h-[2px] w-4 rounded-full bg-foreground" /></span><span className="hidden text-sm font-semibold sm:inline">Menu</span></button>
      </div></div></header>
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-10 lg:py-10"><div className="mb-6 flex flex-wrap items-end justify-between gap-3 lg:mb-8"><div><p className="text-label-caps flex items-center gap-1.5 text-muted-foreground"><Store className="size-3.5" aria-hidden />{store?.name ?? "Loading shop"}{branch ? ` · ${branch.name}` : ""}</p><h1 className="mt-1 text-title-lg lg:text-display-md">{title}</h1></div></div>{children}</div>
