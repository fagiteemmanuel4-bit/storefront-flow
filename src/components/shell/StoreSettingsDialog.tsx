...
export function StoreSettingsDialog({ store, mode, canEdit, open, onOpenChange }: { store: StoreRow | null; mode: Mode; canEdit: boolean; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  useEffect(() => { if (!store || !open) return; setName(store.name); setCurrency(store.currency); setTaxRate(String(store.tax_rate)); setLowStock(String(store.low_stock_threshold)); }, [store, open]);
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("stores").update({ name: name.trim(), currency, tax_rate: tax, low_stock_threshold: Math.round(low) }).eq("id", store.id);
      if (error) throw new Error(error.message);
    },
    onSuccess: async () => { toast.success("Saved"); await queryClient.invalidateQueries(); onOpenChange(false); },
