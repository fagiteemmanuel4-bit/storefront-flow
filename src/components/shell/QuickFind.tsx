...
export function QuickFind({ open, onOpenChange }: QuickFindProps) {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-start justify-center bg-foreground/35 px-3 pt-[10vh] backdrop-blur-[2px]" role="presentation" onMouseDown={() => onOpenChange(false)}>
      <div className="w-full max-w-xl overflow-hidden rounded-2xl border border-border bg-background shadow-2xl" role="dialog" aria-modal="true" aria-label="Find in Strap" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="max-h-[55vh] overflow-y-auto p-2">
          ...
          {results.length ? results.map((action) => {
            ...
            return <button key={action.to} type="button" onClick={() => go(action.to)} className="group flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left hover:bg-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent">
          ...
          }) : <div className="px-3 py-8 text-center"><p className="text-sm font-semibold">No matching Strap destination</p><p className="mt-1 text-xs text-muted-foreground">Try a product, order, customer, report, or action.</p></div>}
        </div>
        <div className="flex items-center justify-between border-t border-border bg-secondary/30 px-4 py-2.5 text-[11px] text-muted-foreground">
          <span className="flex items-center gap-1.5"><Command className="size-3.5" />Quick find</span>
