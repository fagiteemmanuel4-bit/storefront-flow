...
export function PasswordOtp() {
  ...
  async function changePassword(nextOtp = otp) {
    if (nextOtp.length !== OTP_LENGTH || saving) return;
        ...
        <div className="space-y-4">
          ...
          <Input
            ...
            onChange={(event) => {
              ...
              setOtp(next);
              if (next.length === OTP_LENGTH) window.setTimeout(() => void changePassword(next), 0);
            }}
            placeholder="00000000"
            className="h-14 text-center text-2xl font-semibold tracking-[0.32em] select-none"
