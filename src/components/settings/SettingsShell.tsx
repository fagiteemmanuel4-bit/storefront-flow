...
function groups() {
  const result = new Map<string, typeof items[number][]>();
    ...
    <AppShell title="Settings">
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="border-b border-border pb-6">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">Manage your account, store and commerce preferences from one focused workspace.</p>
        </header>
...
export function SettingsHomeShell() {
  return (
    <div className="space-y-6">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {items.map((item) => { const Icon = item.icon; return <Link key={item.path} to={item.path} className="group rounded-2xl border border-border bg-background p-5 transition hover:-translate-y-0.5 hover:border-accent/40 hover:shadow-sm"><div className="flex items-start justify-between"><span className="flex size-10 items-center justify-center rounded-xl bg-accent-soft text-accent-ink"><Icon className="size-5" /></span><ChevronRight className="size-4 text-muted-foreground transition group-hover:translate-x-1" /></div><h2 className="mt-4 font-semibold">{item.title}</h2><p className="mt-1.5 text-sm leading-6 text-muted-foreground">{item.description}</p></Link>; })}
      </div>
      <div className="rounded-2xl border border-border bg-surface p-6"><p className="text-sm leading-6 text-muted-foreground">Settings are saved to Strap's existing Supabase data structures. Payments and custom domains remain unavailable until their production infrastructure is ready.</p></div>
    </div>
  );
}
