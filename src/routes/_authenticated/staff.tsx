...
function StaffPage() {
  ...
  async function createStaff() {
    if (!store?.id) {
      toast.error("No active store is selected.");
      return;
    }
    const trimmedName = name.trim();
    if (!trimmedName || !/^\d{4,6}$/.test(pin)) {
      toast.error("Enter a staff name and a 4–6 digit PIN.");
      return;
    }
    if (!canManage) {
      toast.error("You do not have permission to create staff accounts.");
      return;
    }

    setBusy(true);
    try {
      // Account creation is one server-side transaction. The client never receives
      // or writes a PIN hash, and the success state is reached only after the RPC
      // has committed the staff row and audit record.
      const { data, error } = await (supabase as any).rpc("create_staff_account", {
        _store_id: store.id,
        _name: trimmedName,
        _pin: pin,
        _role: staffRole,
