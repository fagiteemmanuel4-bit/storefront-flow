...
function StockSensePage() {
  ...
  async function analyze(file: File) {
    ...
    try {
      ...
      const worker = await createWorker("eng", 1, { logger: (m) => { if (m.status === "recognizing text" && typeof m.progress === "number") setRawText(`Kudi Sense is reading the sheet… ${Math.round(m.progress * 100)}%`); } });
      try { const { data } = await worker.recognize(file); setRawText(data.text.trim()); const parsed = parseSenseText(data.text); setRows(parsed); if (!parsed.length) toast.error("Kudi could not confidently read a row. Retake the photo with the table flat, bright and easy to read."); else toast.success(`${parsed.length} stock row${parsed.length === 1 ? "" : "s"} detected. Review before saving.`); } finally { await worker.terminate(); }
    } catch (error) { toast.error(errorMessage(error, "Kudi Sense could not read this photo.")); } finally { setBusy(false); }
  }
