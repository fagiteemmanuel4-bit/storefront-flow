...
function OnboardingPage() {
  ...
  const { memberships, isLoading, setActiveStore } = useStoreContext();
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      // Keep the client-side snapshot in step with the change we just made.
      ...
      toast.error(errorMessage(error, "We couldn't create the shop."));
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <div className="min-h-screen bg-background px-4 py-10 sm:px-6">
      <div className="mx-auto max-w-lg">
        <p className="text-label-caps text-accent-ink">Step 1 of 1</p>
        <h1 className="mt-2 text-display-md">Set up your shop</h1>
        <p className="mt-3 text-base text-muted-foreground">
          You can add more locations later — they'll share this product catalogue and keep their own
          stock counts.
        </p>
        ...
        <form onSubmit={handleSubmit} className="surface-card mt-8 space-y-5 p-5 sm:p-6">
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Mama Bisi Provisions"
              className="h-12"
              maxLength={80}
          ...
          <div className="space-y-2">
            <Label htmlFor="branch-name">First location</Label>
            <Input
              id="branch-name"
              value={branchName}
          ...
          <p className="text-xs text-muted-foreground">
            Currency can't be changed after your first sale, so pick the one you actually price in.
          </p>
        </form>
      </div>
    </div>
