...
import { Calculator, CalendarDays, Check, ChevronDown, Clock3, ExternalLink, List, Minus, Plus, Printer, RotateCcw, ScanLine, Search, ShoppingCart, Trash2, UserRound, WifiOff, X } from "lucide-react";
...
function PosPage() {
  const { store, branch, branches, setActiveBranch } = useStoreContext();
  ...
  return <AppShell title="Sell"><div data-tour="sell" className="pos-sell-page">
    <div className="pos-mobile-header lg:hidden">
      <div className="pos-mobile-brand"><span className="pos-brand-mark"><span /></span><span>Strap</span></div>
      <div className="pos-icon-toolbar" aria-label="Sell tools">
        <button type="button" className="pos-toolbar-button" aria-label="Open current sale" onClick={() => setCartOpen(true)}><ShoppingCart className="size-[18px]" />{cart.length > 0 && <span className="pos-cart-count">{cart.reduce((n, l) => n + l.quantity, 0)}</span>}</button>
        <Link to="/online-store/orders" className="pos-toolbar-button" aria-label="Orders"><List className="size-[18px]" /></Link>
        <Link to="/activity" className="pos-toolbar-button" aria-label="Activity calendar"><CalendarDays className="size-[18px]" /></Link>
      </div>
    </div>
    <div className="pos-domain-banner lg:hidden"><span>Strap is moving to a new domain soon</span></div>
    {branches.length > 0 && branch && <div className="mb-3 flex items-center justify-between gap-3 lg:hidden"><Select value={branch.id} onValueChange={setActiveBranch}><SelectTrigger className="h-10 w-auto max-w-full rounded-full border-border bg-surface px-3 text-xs font-semibold shadow-none"><SelectValue /></SelectTrigger><SelectContent>{branches.map((b) => <SelectItem key={b.id} value={b.id}>{store?.name ? `${store.name} · ` : ""}{b.name}</SelectItem>)}</SelectContent></Select><span className="text-[11px] text-muted-foreground">Sell</span></div>}
    <div className="flex flex-wrap items-center gap-2">
      <div className="relative min-w-0 flex-1"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search product, SKU or barcode" className="h-11 rounded-xl border-border bg-surface pl-9 shadow-none" /></div>
      <select aria-label="Filter by category" className="field h-11 w-auto rounded-xl border-border bg-surface" value={category} onChange={(e) => setCategory(e.target.value)}><option value="all">All categories</option>{categories.map((c) => <option key={c} value={c}>{c}</option>)}</select>
      <Button variant="outline" aria-label="Scan barcode" className="h-11 w-11 shrink-0 rounded-xl border-border bg-surface p-0" onClick={() => setScanning(true)}><ScanLine className="size-[18px]" /></Button>
    </div>
    <div className="mt-3 flex flex-wrap items-center gap-2">
      <div className="flex items-center gap-2 rounded-full border border-border bg-surface px-3 py-2 text-xs font-medium"><span className={cn("size-2 rounded-full", online && offlineCount === 0 ? "bg-emerald-500" : online ? "bg-amber-500" : "bg-red-500")} />{online ? (offlineCount > 0 ? `${offlineCount} sale${offlineCount === 1 ? "" : "s"} waiting to sync` : "Online · sales sync normally") : "Offline · sales are queued on this device"}</div>
      <Button variant="ghost" size="sm" onClick={holdSale} disabled={!cart.length} className="rounded-full px-3 text-muted-foreground"><Clock3 className="mr-1.5 size-4" />Hold sale</Button>
      {Object.keys(holds).length > 0 && <details className="relative"><summary className="cursor-pointer list-none rounded-full border border-border bg-surface px-3 py-2 text-xs font-semibold">Held sales ({Object.keys(holds).length})</summary><div className="absolute left-0 top-10 z-40 w-64 rounded-2xl border border-border bg-surface p-2 shadow-xl">{Object.keys(holds).map((name) => <button key={name} className="flex w-full items-center justify-between rounded-xl px-3 py-2 text-left text-sm hover:bg-secondary" onClick={() => recallSale(name)}><span className="truncate">{name}</span><RotateCcw className="size-3.5" /></button>)}</div></details>}
      <Link to="/receipt-designer" target="_blank" className="ml-auto inline-flex items-center gap-1.5 rounded-lg px-2 py-2 text-xs font-semibold text-muted-foreground hover:bg-secondary hover:text-foreground"><Printer className="size-3.5" />Receipt designer <ExternalLink className="size-3" /></Link>
    </div>
