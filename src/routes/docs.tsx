...
import { ArrowRight, BarChart3, BookOpen, Boxes, CheckCircle2, ChevronRight, Code2, Globe2, KeyRound, Package, Receipt, Rocket, Search, ShieldCheck, ShoppingBag, Store, Users, Zap } from "lucide-react";
...
function DocSection({ id, icon: Icon, eyebrow, title, children }: { id: string; icon: typeof Rocket; eyebrow: string; title: string; children: ReactNode }) { return <section id={id} className="scroll-mt-24 border-b border-border py-12 first:pt-0"><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[.14em] text-accent-ink"><Icon className="size-4" />{eyebrow}</div><h2 className="mt-3 font-display text-3xl font-bold tracking-tight">{title}</h2><div className="prose prose-neutral mt-5 max-w-none text-[15px] leading-7 text-muted-foreground [&_p]:max-w-3xl [&_h3]:text-foreground">{children}</div></section>; }
