...
export const Route = createFileRoute("/auth")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Sign in to Kudi or create an account for your shop." },
    ],
  }),
  component: AuthPage,
});
...
function AuthPage() {
    ...
    try {
      ...
      if (mode === "signup") {
        const { data, error } = await supabase.auth.signUp({
          ...
          options: {
            emailRedirectTo: `${window.location.origin}/verified?kind=email&next=${encodeURIComponent("/auth")}`,
      ...
      if (error) {
        if (error.message.toLowerCase().includes("email not confirmed")) {
          setNotice("Your email is not verified yet. Open the verification link we sent to you, then sign in again.");
        }
  ...
  const passwordFieldVisible = mode !== "reset";
    ...
    <div className="flex min-h-screen flex-col bg-background px-4 py-10 sm:px-6">
      ...
      <div className="mx-auto mt-10 w-full max-w-md">
        <div className="mb-6 text-center">
          <div className="mx-auto flex size-12 items-center justify-center rounded-2xl border border-border bg-surface shadow-lift"><ShieldCheck className="size-5" /></div>
          <h1 className="text-display-md mt-5">{title}</h1>
        ...
        <form onSubmit={handleSubmit} className="surface-card space-y-4 rounded-3xl border p-5 shadow-lift sm:p-6">
          ...
          <div className="space-y-2"><Label htmlFor="email">Email</Label><Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="h-12 rounded-xl" autoComplete="email" disabled={mode === "recover"} required /></div>
