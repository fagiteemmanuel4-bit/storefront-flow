...
export const Route = createFileRoute("/auth")({ ssr: false, head: () => ({ meta: [{ title: "Sign in or create your shop — Strap" }, { name: "description", content: "Sign in to Strap or create an account for your shop." }] }), component: AuthPage });
...
function AuthPage() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    if (mode !== "recover") {
      ...
      try {
        ...
        // First validate the password. Supabase may create a session here, so immediately
        // revoke it before sending the one-time code required for final sign-in.
  ...
  const subtitle = mode === "signin" ? "Sign in with your password, then confirm a one-time code sent to your email." : mode === "signup" ? "Create your account, then verify it with a one-time security code." : mode === "reset" ? "We'll send a secure verification link to your email." : "Use the secure link we sent you to finish changing your password.";

  return <div className="flex min-h-screen flex-col bg-background px-4 py-10 sm:px-6"><Link to="/" className="mx-auto flex items-center gap-2"><span className="flex size-8 items-center justify-center rounded-full bg-accent"><span className="size-3 rotate-45 rounded-[3px] bg-foreground" /></span><span className="font-display text-xl font-bold tracking-tight">STRAP.</span></Link><div className="mx-auto mt-10 w-full max-w-md"><div className="mb-6 text-center"><div className="mx-auto flex size-12 items-center justify-center rounded-2xl border border-border bg-surface shadow-lift"><ShieldCheck className="size-5" /></div><h1 className="text-display-md mt-5">{title}</h1><p className="mt-2 text-muted-foreground">{subtitle}</p></div><form onSubmit={handleSubmit} className="surface-card space-y-4 rounded-3xl border p-5 shadow-lift sm:p-6">{mode === "signup" && <div className="space-y-2"><Label htmlFor="name">Your name</Label><Input id="name" value={fullName} onChange={(e) => setFullName(e.target.value)} className="h-12 rounded-xl" autoComplete="name" maxLength={120} required /></div>}{mode !== "recover" && <div className="space-y-2"><Label htmlFor="email">Email</Label><Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="h-12 rounded-xl" autoComplete="email" required /></div>}{mode !== "reset" && <div className="space-y-2"><div className="flex items-center justify-between"><Label htmlFor="password">{mode === "recover" ? "New password" : "Password"}</Label>{mode === "signin" && <button type="button" className="text-sm font-semibold text-accent-ink underline underline-offset-4" onClick={() => { setMode("reset"); setNotice(null); }}>Forgot password?</button>}</div><div className="relative"><Input id="password" type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} className="h-12 rounded-xl pr-11" autoComplete={mode === "signin" ? "current-password" : "new-password"} required /><button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute right-2 top-1/2 inline-flex size-8 -translate-y-1/2 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground" aria-label={showPassword ? "Hide password" : "Show password"}>{showPassword ? <EyeOff className="size-4" /> : <Eye className="size-4" />}</button></div></div>}{mode === "signup" && <label className="flex items-start gap-3 rounded-xl bg-secondary/50 p-3 text-sm text-muted-foreground"><Checkbox checked={accepted} onCheckedChange={(value) => setAccepted(value === true)} className="mt-0.5" aria-label="Accept terms and privacy policy" /><span>I accept the <Link to="/terms" className="font-semibold text-accent-ink underline">Terms</Link> and <Link to="/privacy" className="font-semibold text-accent-ink underline">Privacy Policy</Link>.</span></label>}{notice && <div className="rounded-xl border border-border bg-secondary/60 p-3 text-sm text-muted-foreground">{notice}</div>}<Button type="submit" className="h-12 w-full rounded-xl" disabled={busy}>{busy ? "Working…" : mode === "signin" ? "Continue securely" : mode === "signup" ? "Create account" : mode === "reset" ? "Send verification link" : "Save new password"}</Button>{(mode === "signin" || mode === "signup") && <div className="flex items-center justify-center gap-2 text-xs text-muted-foreground"><ShieldCheck className="size-3.5" /> Password + one-time email code verification</div>}</form>{(mode === "signin" || mode === "signup") && <p className="mt-5 text-center text-sm text-muted-foreground">{mode === "signin" ? "New here?" : "Already have an account?"} <button type="button" className="font-semibold text-foreground underline" onClick={() => { setMode(mode === "signin" ? "signup" : "signin"); setNotice(null); }}>{mode === "signin" ? "Create an account" : "Sign in instead"}</button></p>}{(mode === "reset" || mode === "recover") && <button type="button" className="mt-5 block w-full text-center text-sm font-semibold text-foreground underline" onClick={() => { setMode("signin"); setNotice(null); }}>Back to sign in</button>}</div></div>;
}