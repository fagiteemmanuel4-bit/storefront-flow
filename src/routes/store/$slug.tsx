...
import { ArrowRight, Check, Mail, Minus, Phone, Plus, ShoppingBag, Trash2 } from "lucide-react";
...
function PublicStorefrontPage() {
  ...
  if (catalogQuery.isLoading) return <main className="flex min-h-screen items-center justify-center bg-background"><span className="size-8 animate-spin rounded-full border-[3px] border-foreground border-t-transparent" aria-label="Loading storefront" /></main>;
  ...
  return <main className="storefront-shell min-h-screen bg-background">
    ...
    <section className="mx-auto max-w-6xl px-5 pb-12 pt-14 lg:px-8 lg:pb-16 lg:pt-20">
      <div className="storefront-card relative overflow-hidden p-7 sm:p-10 lg:p-14">
        <div className="pointer-events-none absolute -right-24 -top-24 size-72 rounded-full bg-accent-soft blur-3xl" />
        <div className="relative max-w-3xl">
          <p className="text-label-caps text-accent-ink">{store.display_name}</p>
          <h2 className="text-display-lg mt-4">Everything you need, in one place.</h2>
          <p className="mt-6 max-w-2xl text-base leading-7 text-muted-foreground sm:text-lg">{store.store_description || "Browse our latest products and place an order directly with the store."}</p>
          <div className="mt-7 flex flex-wrap gap-3">{store.display_phone && <span className="inline-flex items-center gap-2 rounded-full border border-border bg-secondary/60 px-3 py-2 text-sm text-muted-foreground"><Phone className="size-4" />{store.display_phone}</span>}{store.display_email && <span className="inline-flex items-center gap-2 rounded-full border border-border bg-secondary/60 px-3 py-2 text-sm text-muted-foreground"><Mail className="size-4" />{store.display_email}</span>}</div>
        </div>
      </div>
    </section>
    ...
    <section className="mx-auto max-w-6xl px-5 pb-24 lg:px-8">
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{catalog.map((product) => <article key={product.product_id} className="storefront-card group">
        <div className="aspect-square overflow-hidden bg-secondary">{product.image_url ? <img src={product.image_url} alt={product.name} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /> : <div className="flex h-full items-center justify-center"><ShoppingBag className="size-10 text-muted-foreground" /></div>}</div>
