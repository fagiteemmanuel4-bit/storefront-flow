...
export const Route = createFileRoute("/store/$slug/products/$productSlug")({
  ssr: true,
...
function PublicProductPage() {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await onlineSupabase.rpc("get_public_store_product", {
        _store_slug: slug,
  ...
  const storeJsonLd = buildStoreJsonLd({
    ...
    description: product.store_description ?? undefined,
    logo: product.store_logo_url ?? undefined,
    url: `/store/${encodeURIComponent(product.store_slug)}`,
  });
    ...
    <div className="mx-auto max-w-6xl px-5 py-8 lg:px-8 lg:py-12">
      ...
      <section className="grid gap-10 lg:grid-cols-[minmax(0,1.08fr)_minmax(360px,.92fr)] lg:items-start">
        ...
        <article className="lg:sticky lg:top-8">
          <p className="text-label-caps text-muted-foreground">{product.product_category || "Product"}</p>
          <h1 className="text-display-lg mt-3">{product.product_name}</h1>
          <p className="numeric mt-5 text-2xl font-bold">{formatMoney(product.product_price, product.currency)}</p>
          <p className={`mt-2 text-sm font-semibold ${product.available ? "text-success" : "text-destructive"}`}>{product.available ? `${product.stock_quantity} available` : "Currently out of stock"}</p>
          {product.product_description && <div className="mt-7 border-t border-border pt-7"><h2 className="font-display text-lg font-semibold">About this product</h2><p className="mt-3 whitespace-pre-line text-sm leading-7 text-muted-foreground">{product.product_description}</p></div>}
          <div className="mt-7 border-t border-border pt-7"><p className="text-xs text-muted-foreground">Sold by</p><Link to="/store/$slug" params={{ slug }} className="mt-1 inline-block font-semibold hover:underline">{product.store_name}</Link>{product.product_sku && <p className="mt-3 text-xs text-muted-foreground">SKU: {product.product_sku}</p>}</div>
          <Link to="/store/$slug" params={{ slug }} className="mt-8 inline-flex"><Button size="lg" className="rounded-xl">Continue shopping</Button></Link>
        </article>
      </section>
    </div>
