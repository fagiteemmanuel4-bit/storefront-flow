-- Kudi staff creation hardening.
-- Staff accounts already exist in production; this migration makes creation atomic,
-- validates the input server-side, prevents duplicate names/PIN reuse, and records an audit event.

CREATE UNIQUE INDEX IF NOT EXISTS staff_accounts_store_name_unique_idx
  ON public.staff_accounts (store_id, lower(trim(name)));
...
BEGIN
  ...
  FOR v_existing IN SELECT pin_hash FROM public.staff_accounts WHERE store_id = _store_id AND is_active = true LOOP
  ...
  RETURN QUERY SELECT s.id, s.name, s.role, s.is_active
  FROM public.staff_accounts s WHERE s.id = v_staff_id;
END;
$$;
